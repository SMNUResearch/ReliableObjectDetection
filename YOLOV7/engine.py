        ...
        for i, (image, shapes, path, image_index) in enumerate(test_loader):
            ...
            for index, p in enumerate(output):
                ...
                for j, b in zip(p.tolist(), box.tolist()):
                    output_file.append({'image_id': image_id, 'category_id': coco_class_id[int(j[5])], 'bbox': [round(x, 3) for x in b], 'score': round(j[4], 5), 'R': round(j[6], 5)})
    ...
    with torch.no_grad():
        for i, (image, shapes, path, image_index) in enumerate(test_loader):
            ...
            for index, p in enumerate(output):
                ...
                for j, b in zip(p.tolist(), box.tolist()):
                    output_file.append({'image_name': image_name, 'category_id': int(j[5]), 'bbox': [round(x, 3) for x in b], 'score': round(j[4], 5), 'bbox_area': round((b[3] * b[2]), 5), 'R': round(j[6], 5)})

    predictions_file = os.path.join(opt.OUTPUT_PATH, 'PRED.json')
    with open(predictions_file, 'w') as f:
        json.dump(output_file, f)
